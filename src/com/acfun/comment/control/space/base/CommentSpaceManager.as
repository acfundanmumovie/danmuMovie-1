package com.acfun.comment.control.space.base
{
	...
	import com.acfun.comment.display.base.Comment;
		...
		public class CommentSpaceManager
		{
			...
			/////////////设置顶部先显示还是底部先显示
			public static var topShow:Boolean = false;
			...
			protected function setY(cmt:Comment,index:int = 0):void
			{
				...
				{
					cmt.setY(0,index,transformY);
					Util.binsert(pool,cmt,bottom_cmp);
					return;
				}
			...
			protected function transformY(y:int,cmt:Comment):int
			{
				if(!topShow)//底部先显示弹幕
				{
					var a:int = ConstValue.STAGE.stageHeight - y -cmt.height;
					return a;
				}
